...
public:
    ...
    uint32_t out_states;
    uint8_t out_letters;
    uint16_t out_directions;

    TransFn(uint32_t out_states, uint8_t out_letters, uint16_t out_directions)
        : out_states(out_states), out_letters(out_letters), out_directions(out_directions)
    {}
    bool is_valid();
